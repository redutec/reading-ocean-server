...
/**
 * ReadingGameContestLevelQuestion 엔티티
 * 테이블 정보:
 * - 테이블 명: reading_game_contest_level_question
 * - 테이블 코멘트: 독서 게임 대회 레벨 진단 항목
 */
@Entity
@Table(name = "reading_game_contest_level_question")
@Getter
@Builder
@NoArgsConstructor(access = AccessLevel.PROTECTED)
@AllArgsConstructor(access = AccessLevel.PRIVATE)
@EntityListeners(AuditingEntityListener.class)
public class ReadingGameContestLevelQuestion {
    @Id
    @GeneratedValue(strategy = GenerationType.IDENTITY)
    @Column(name = "reading_game_contest_level_question_no", nullable = false, updatable = false)
    private Integer readingGameContestLevelQuestionNo;

    @Column(name = "reading_game_contest_level_no", nullable = false)
