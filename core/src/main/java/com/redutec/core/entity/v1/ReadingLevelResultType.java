...
/**
 * ReadingLevelResultType 엔티티
 * 테이블 정보:
 * - 테이블 명: reading_level_result_type
 * - 테이블 코멘트: 독서 레벨 진단 결과지 결과 타입
 */
@Entity
@Table(name = "reading_level_result_type")
@Getter
@Builder
@NoArgsConstructor(access = AccessLevel.PROTECTED)
@AllArgsConstructor(access = AccessLevel.PRIVATE)
@EntityListeners(AuditingEntityListener.class)
public class ReadingLevelResultType {
    @Id
    @GeneratedValue(strategy = GenerationType.IDENTITY)
    @Column(name = "reading_level_result_type_no", nullable = false, updatable = false)
    private Integer readingLevelResultTypeNo;

    @Column(name = "reading_level_result_no", nullable = false)
