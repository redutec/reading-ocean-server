...
public class StudentShipping {
    @Id
    @GeneratedValue(strategy = GenerationType.IDENTITY)
    @Comment("배송 고유번호")
    private Long id;

    @Comment("연결된 주문(학생) 엔티티")
    ...
    @OneToMany(
            mappedBy = "studentShipping",
            cascade = CascadeType.ALL,
            orphanRemoval = true
    )
    @OrderBy("createdAt ASC")
    private List<StudentShippingLog> shippingLogs = new ArrayList<>();
    ...
    public void changeStatus(ShippingStatus newStatus, String note) {
        ...
        this.shippingLogs.add(
                StudentShippingLog.builder()
                        .studentShipping(this)
                        .status(newStatus)
                        .note(note)
                        .build()
        );
