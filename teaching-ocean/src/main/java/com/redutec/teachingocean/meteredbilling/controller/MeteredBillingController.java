package com.redutec.teachingocean.meteredbilling.controller;

import com.redutec.teachingocean.meteredbilling.service.MeteredBillingService;
...
public class MeteredBillingController {
    ...
    @Operation(summary = "현재 로그인한 교사가 속한 교육기관의 월별 사용료 청구서 목록 조회", description = "현재 로그인한 교사가 속한 교육기관의 월별 사용료 청구서 목록을 조회하는 API")
    @GetMapping
    public ResponseEntity<ApiResponseBody> find(
            @ParameterObject @Valid MeteredBillingDto.FindMeteredBillingRequest findMeteredBillingRequest
    ) {
        return apiResponseManager.ok(meteredBillingService.find(findMeteredBillingRequest));
    }

    @Operation(summary = "현재 로그인한 교사가 속한 교육기관의 특정 월별 사용료 청구서 조회", description = "특정 월별 사용료 청구서를 조회하는 API")
