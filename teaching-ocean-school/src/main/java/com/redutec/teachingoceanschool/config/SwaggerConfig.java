package com.redutec.teachingoceanschool.config;
...
public class SwaggerConfig {
	 ...
	 */
	@Bean
	public OpenAPI teachingOceanSchoolAPI() {
		// API 정보 설정
		Info info = new Info()
				.title("Teaching Ocean School API")
				.version("v2")
				.description("Teaching Ocean School API Collection");
		// OpenAPI 객체 생성 및 설정
		return new OpenAPI()
				.addServersItem(new Server().url("/api/to-school"))
	...
	public OpenApiCustomizer globalResponseCustomizer() {
		return openApi -> openApi.getPaths().values().forEach(pathItem -> pathItem.readOperations().forEach(operation -> {
			...
			apiResponses.addApiResponse("409", createApiResponse("Conflict - 요청이 충돌됨"));
