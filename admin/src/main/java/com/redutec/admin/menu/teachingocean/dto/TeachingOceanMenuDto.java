package com.redutec.admin.menu.teachingocean.dto;
...
public class TeachingOceanMenuDto {
    @Schema(description = "티칭오션 메뉴 등록 요청 객체")
    public record CreateTeachingOceanMenuRequest(
    ...
    @Schema(description = "티칭오션 메뉴 조회 요청 객체")
    public record FindTeachingOceanMenuRequest(
            @Schema(description = "티칭오션 메뉴 ID", requiredMode = Schema.RequiredMode.NOT_REQUIRED)
            List<@Positive Long> teachingOceanMenuIds,
    ...
    @Schema(description = "티칭오션 메뉴 수정 요청 객체")
    public record UpdateTeachingOceanMenuRequest(
            @Schema(description = "메뉴명", requiredMode = Schema.RequiredMode.NOT_REQUIRED)
            @Size(max = 50, message = "메뉴명은 50자를 넘을 수 없습니다")
            String name,
    ...
    @Schema(description = "티칭오션 메뉴 응답 객체")
    public record TeachingOceanMenuResponse(
            ...
            List<TeacherRole> accessibleRoles,
            Integer depth,
            TeachingOceanMenuDto.TeachingOceanMenuResponse parentMenu,
            List<TeachingOceanMenuDto.TeachingOceanMenuResponse> childrenMenus,
            LocalDateTime createdAt,
            LocalDateTime updatedAt
    ) {}

    @Schema(description = "티칭오션 메뉴 응답 페이징 객체")
    public record TeachingOceanMenuPageResponse(
            List<TeachingOceanMenuResponse> teachingOceanMenus,
            Long totalElements,
            Integer totalPages
    ) {}
}