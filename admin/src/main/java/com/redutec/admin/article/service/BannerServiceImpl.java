...
import com.redutec.admin.article.mapper.BannerMapper;
...
public class BannerServiceImpl implements BannerService {
    ...
    public BannerDto.BannerResponse findByBannerNo(
        ...
        BannerDto.BannerAttachFileResponse pcImageFile =
                Optional.ofNullable(articleService.findBdtArticleAttachFileByArticleAndAttachFileValue(banner, AttachFileValue.AFV002))
        ...
        BannerDto.BannerAttachFileResponse mobileImageFile =
                Optional.ofNullable(articleService.findBdtArticleAttachFileByArticleAndAttachFileValue(banner, AttachFileValue.AFV001))
    ...
    public void update(
        ...
        BdtArticle banner = articleService.getBdtArticleByArticleNo(bannerNo);
        BdtArticleDisplay bannerDisplay = articleService.findBdtArticleDisplayByArticle(banner);
        BdtArticleAttachFile pcBannerAttachFile = articleService.findBdtArticleAttachFileByArticleAndAttachFileValue(banner, AttachFileValue.AFV002);
        BdtArticleAttachFile mobileBannerAttachFile = articleService.findBdtArticleAttachFileByArticleAndAttachFileValue(banner, AttachFileValue.AFV001);
        // PC용 배너 이미지 파일 검증
        Optional.ofNullable(pcBannerImageFile)
                .filter(file -> !file.isEmpty())
                .ifPresent(file -> fileUtil.validateFile(file, MAX_FILE_SIZE, ALLOWED_EXTENSIONS));
