...
public class StudentSubscriptionController {
    ...
    @PostMapping
    public ResponseEntity<ApiResponseBody> create(@ParameterObject @Valid StudentSubscriptionDto.CreateStudentSubscriptionRequest createStudentSubscriptionRequest) {
    ...
    @GetMapping
    public ResponseEntity<ApiResponseBody> find(@ParameterObject @Valid StudentSubscriptionDto.FindStudentSubscriptionRequest findStudentSubscriptionRequest) {
    ...
    @GetMapping("/{studentSubscriptionId}")
    public ResponseEntity<ApiResponseBody> findById(@PathVariable Long studentSubscriptionId) {
        return apiResponseManager.ok(subscriptionStudentService.findById(studentSubscriptionId));
    }

    @Operation(summary = "특정 구독(학생) 수정", description = "특정 구독(학생) 정보를 수정하는 API")
    @PutMapping("/{studentSubscriptionId}")
    public ResponseEntity<ApiResponseBody> update(
            @Parameter(description = "구독(학생) ID") @PathVariable Long studentSubscriptionId,
            @ParameterObject @Valid StudentSubscriptionDto.UpdateStudentSubscriptionRequest updateStudentSubscriptionRequest
    ) {
        subscriptionStudentService.update(studentSubscriptionId, updateStudentSubscriptionRequest);
        return apiResponseManager.noContent();
    }

    @Operation(summary = "특정 구독(학생) 삭제", description = "특정 구독(학생) 정보를 삭제하는 API")
    @DeleteMapping("/{studentSubscriptionId}")
    public ResponseEntity<ApiResponseBody> delete(@Parameter(description = "구독(학생) ID") @PathVariable Long studentSubscriptionId) {
        subscriptionStudentService.delete(studentSubscriptionId);
        return apiResponseManager.noContent();
    }
}